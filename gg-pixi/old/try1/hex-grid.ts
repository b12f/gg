export class TileIterator {
    ...
    public next() {
        if (this.tileIdx >= this.hexGrid.tiles.length) {
            return null;
        }

        var tile = this.hexGrid.tiles[this.tileIdx];
        this.tileIdx += 1;
        return tile;
    }
}
...
export interface AbstractTileFactory {
    newTile: () => AbstractTile;
}
...
export interface HexGridOptions {
    ...
    orientation: 'flat-topped'|'pointy-topped';
...
export class HexGrid {
    ...
    constructor(options: HexGridOptions) {
        if (VALID_LAYOUTS[options.orientation].indexOf(options.layout) === -1) {
            throw new Error('Invalid options.layout for the chosen orientation. Must '+
                'be one of: ' + VALID_LAYOUTS[options.orientation]);
        }
     ...
     */
    public isValidDirection(dir) {
        if (VALID_LAYOUTS[this.orientation].indexOf(dir) === -1) {
            return false;
        }
     ...
     */
    public getNeighbourByCoords(x, y, dir) {
     ...
     * @return {object|null} The neighbouring tile.
     */
    public getNeighbourById(tileId, dir) {
     ...
     */
    public getNeighboursById(tileId) {
        var coords = this.getCoordsById(tileId);
        if (coords === null) {
            throw new Error('Invalid tile ID: ' + tileId);
        }

        return VALID_LAYOUTS[this.orientation].map(function (dir) {
            return this.getNeighbourByCoords(coords.x, coords.y, dir);
     ...
     */
    public getPositionByCoords(x, y) {
        var xPos = x,
            yPos = y;
     ...
     */
    public getShortestPathsFromTileId = function(tileId, options) {
